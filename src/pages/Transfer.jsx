...
export default function Transfer() {
  ...
  const initiateTransfer = async () => {
    ...
    setError('');
    try {
      const otpResponse = await transferApi.generateOTP({
        phone: '+2348000000000',
      ...
      setTransactionRef(sessionId);
      setFormData(prev => ({ ...prev, sessionId }));
      setStep(4);
      startTimer();
      setOtpStatus('OTP sent to your phone/email');
  ...
  const resendOTP = async () => {
    ...
    try {
      const sessionId = formData.sessionId || transactionRef;
      const response = await transferApi.resendOTP({ sessionId });
      if (response.success) {
      ...
      case 1:
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Recipient Bank / Mobile Money</label>
              <select name="recipientBank" value={formData.recipientBank} onChange={handleChange}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Recipient Account Number</label>
              <input type="text" name="recipientAccount" value={formData.recipientAccount} onChange={handleChange}
      ...
      case 2:
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Amount (NGN)</label>
      ...
      case 3:
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Sender Bank</label>
              <select name="senderBank" value={formData.senderBank} onChange={handleChange}
                className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Sender Account Number</label>
              <input type="text" name="senderAccount" value={formData.senderAccount} onChange={handleChange}
                placeholder="Enter 10-digit account number" maxLength={10}
                className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" name="otp" value={formData.otp} onChange={handleChange}
                ...
                className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <p className="text-xs text-gray-500 mt-1">Enter the OTP sent to your registered phone/email by your bank</p>
            </div>
            {timer > 0 && <p className="text-sm text-gray-400">⏱️ OTP valid for: {timer}s</p>}
            ...
            {otpStatus && <p className="text-green-400 text-sm">{otpStatus}</p>}
          </div>
        );
      default:
        return null;
  ...
  return (
    <div className="max-w-md mx-auto p-4 min-h-screen">
        ...
        {STEPS.map((s) => (
          <div key={s.id} className="flex flex-col items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
              s.id === step ? 'bg-blue-500 text-white' : s.id < step ? 'bg-green-500 text-white' : 'bg-gray-700 text-gray-400'
            }`}>
              {s.id < step ? '✓' : s.id}
            </div>
            <span className={`text-xs mt-1 ${s.id === step ? 'text-blue-400' : 'text-gray-500'}`}>{s.label}</span>
          </div>
        ))}
      </div>

      {error && <div className="bg-red-900/50 border border-red-700 text-red-200 p-3 rounded-lg mb-4 text-sm">⚠️ {error}</div>}
      ...
      <div className="flex gap-3 mt-4">
        {step > 1 && step !== 4 && (
          <button onClick={prevStep} className="flex-1 py-3 bg-gray-700 hover:bg-gray-600 rounded-lg font-medium transition">Back</button>
        )}
        <button onClick={step === 4 ? confirmTransfer : nextStep} disabled={loading}
          className={`flex-1 py-3 rounded-lg font-medium transition ${loading ? 'bg-blue-700 opacity-50' : step === 4 ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
          {loading ? <span className="flex items-center justify-center gap-2"><span className="animate-spin">⏳</span> Processing...</span>
          : step === 4 ? `Pay ₦${formData.amount || '0.00'}` : 'Next →'}
        </button>
      </div>

      {step === 4 && (
        <button className="w-full mt-3 py-2 text-sm text-gray-400 hover:text-white transition">💬 Chat to Edit</button>
      )}
    </div>
  );
    }
